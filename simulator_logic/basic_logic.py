...
class Simulator:
    def __init__(self, user_interface, for_test: bool):
        if not for_test:
            ...
            self.suggestions_in_the_text \
                = worker.split_string_into_substrings(
                  self.text, max_line_length=120)
            self.suggestions_in_the_text.reverse()
            self.current_suggestions_on_display \
                = self.suggestions_in_the_text.pop()
    ...
    def activity(self, key: str, number_key: int) -> None:
        if not self.the_end:
            ...
            if key == self.current_symbols:
                ...
                if len(self.symbols) != 0:
                    ...
                    if language != self.language:
                        self.window.update_the_keyboard_layout(
    ...
    def stop_the_workout(self) -> None:
    ...
    def read_time_from_the_timer(self) -> None:
    ...
    def update_statistic_data(self) -> None:
    ...
    def update_line(self, new_line: str) -> None:
